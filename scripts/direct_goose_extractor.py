#!/usr/bin/env python3
"""
Direct Goose Extractor for Regional Accent Analysis
Extracts segments containing 'goose' directly from audio files.
"""
...
class DirectGooseExtractor:
    """Extract goose segments directly from audio files."""
...
def main():
    ...
    if len(sys.argv) < 3:
        print("Usage: python direct_goose_extractor.py <json_directory> <output_directory>")
        print()
        print("Examples:")
        print("  python direct_goose_extractor.py all_states_comma_story/ goose_segments/")
        sys.exit(1)
    
    json_dir = Path(sys.argv[1])
    output_dir = Path(sys.argv[2])
    
    extractor = DirectGooseExtractor()
